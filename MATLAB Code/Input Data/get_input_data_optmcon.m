...
function outputs = get_input_data_optmcon(r,Hw)
...
input.Hw = Hw;  % microwave irradiation, default 242345
input.ha = 0;  % heat transfer coefficient at the bottom surface (W/m2K)
input.hb = 65;  % heat transfer coefficient at the top surface (W/m2K), default 65
input.pbw = 0.04;
input.pw = 0.92;
input.pice = 1-input.pbw;
input.L = 0.042;  % length/height of the microwave (m)
...
input.r = r;  % shelf temperature ramp-up rate (K/s), default 1/60
...
input.kappa2 = @(x) (input.L*(input.Tb0*input.hb - input.Tm*input.hb + x*input.pw*input.L))/(input.alpf*input.dHsub*input.pice*(input.rhof-input.rhoa));
...
%% Exact solution
input.nterm = 10;  % number of terms in the infinite series
