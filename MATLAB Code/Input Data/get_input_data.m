...
input.kf = 2.30;  % thermal conductivity of the frozen region (W/m2K)
...
input.ha = 0;  % heat transfer coefficient at the top surface (W/m2K)
input.hb = 65;  % heat transfer coefficient at the bottom surface (W/m2K), default 65
input.pbw = 0.04;
input.pw = 0.92;
input.pice = 1-input.pbw;
input.L = 0.042;  % length/height of the product (m)
...
input.r = 1/60;  % shelf temperature ramp-up rate (K/s), default 1/60
...
%input.kappa = @(T) (input.L*(T*input.hb - input.Tm*input.hb + input.Hw*input.pw*input.L))/(input.alpf*input.dHsub*input.pice*(input.rhof-input.rhoa));
...
%% Exact solution
input.nterm = 50;  % number of terms in the infinite series
